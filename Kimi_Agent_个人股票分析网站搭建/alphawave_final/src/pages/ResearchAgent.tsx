...
export default function ResearchAgent() {
  ...
  const [fileName, setFileName] = useState('中科曙光2024年年度报告（已载入基准）');
  const [status, setStatus] = useState<'ready' | 'processing' | 'reviewed' | 'written' | 'error'>('ready');
  const [message, setMessage] = useState('已载入待复核数据；确认来源后可写入三表模型。');
  ...
  const impactItems = useMemo(() => {
    const cases: Array<{ label: string; key: keyof OperatingAssumptions; change: number; }> = [
      ...
      { label: '扩张性资本开支率 +1pct', key: 'growthCapexPct', change: 0.01 },
  ...
  return <div className="mx-auto max-w-[1540px] space-y-4">
    ...
    <section className="grid gap-4 xl:grid-cols-[360px_1fr]">
      <aside className="space-y-4">
        <div className="panel p-4"><h2 className="flex items-center gap-2 text-sm font-semibold text-t-textBright"><Upload className="h-4 w-4 text-t-blue" />上传并解析</h2><label className="mt-4 flex cursor-pointer flex-col items-center justify-center rounded-lg border border-dashed border-t-cyan/35 bg-t-cyan/[0.025] px-4 py-7 text-center hover:bg-t-cyan/[0.05]"><FileText className="h-7 w-7 text-t-cyan" /><span className="mt-3 text-xs font-medium text-t-textBright">选择PDF或文本文件</span><span className="mt-1 text-[10px] text-t-textDim">仅在当前浏览器处理，不保存文件</span><input type="file" accept=".pdf,.txt,.csv,text/plain,application/pdf" onChange={handleFile} className="sr-only" /></label><div className="mt-3 rounded border border-t-border bg-white/[0.012] p-3"><div className="truncate text-xs text-t-text">{fileName}</div><div className={`mt-1 text-[10px] leading-4 ${status === 'error' ? 'text-t-yellow' : 'text-t-textDim'}`}>{message}</div></div><button onClick={() => { setMetrics(sampleMetrics); setFileName('中科曙光2024年年度报告（已载入基准）'); setStatus('ready'); setAuditTime(''); setMessage('基准数据已恢复，请进行人工复核。'); }} className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-md border border-t-border py-2 text-xs text-t-text"><RefreshCw className="h-3.5 w-3.5" />重新载入基准</button></div>
    ...
    <section className="grid gap-4 xl:grid-cols-[1fr_440px]">
      <div className="panel p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="flex items-center gap-2 text-sm font-semibold text-t-textBright"><Sparkles className="h-4 w-4 text-t-yellow" />模型解释 Agent</h2><p className="mt-1 text-[10px] text-t-textDim">基于当前历史基准、经营假设和DCF动态计算</p></div><div className="font-mono text-xs text-t-cyan">Base ¥{dcf.pricePerShare.toFixed(2)}/股</div></div><div className="mt-4 flex gap-2"><div className="relative flex-1"><Search className="absolute left-3 top-2.5 h-3.5 w-3.5 text-t-textDim" /><input value={question} onChange={event => setQuestion(event.target.value)} onKeyDown={event => { if (event.key === 'Enter' && question.trim()) setAnswerQuestion(question); }} className="w-full rounded-md border border-t-border bg-t-bg py-2 pl-9 pr-3 text-xs text-t-text outline-none focus:border-t-cyan/50" /></div><button onClick={() => question.trim() && setAnswerQuestion(question)} className="rounded-md bg-t-cyan px-4 py-2 text-xs font-medium text-slate-950">分析</button></div><div className="mt-3 flex flex-wrap gap-2">{['哪项假设对估值影响最大？', '毛利率下降1个百分点会怎样？', 'WACC提高0.5个百分点会怎样？', 'Bear与Base情景差异是什么？'].map(item => <button key={item} onClick={() => { setQuestion(item); setAnswerQuestion(item); }} className="rounded-full border border-t-border px-2.5 py-1 text-[10px] text-t-textDim hover:border-t-cyan/40 hover:text-t-text">{item}</button>)}</div><div className="mt-4 rounded-lg border border-t-cyan/25 bg-t-cyan/[0.035] p-4"><div className="flex items-center gap-2 text-xs font-medium text-t-cyan"><Bot className="h-4 w-4" />分析结果</div><p className="mt-3 text-sm leading-7 text-t-text">{explanation}</p></div></div>
