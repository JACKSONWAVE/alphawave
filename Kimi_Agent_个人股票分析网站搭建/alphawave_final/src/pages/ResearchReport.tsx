    ...
    <article className="research-report bg-white text-slate-900 shadow-2xl">
      ...
      <div className="space-y-9 px-8 py-8 sm:px-12">
        ...
        <section className="report-page-break">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-3"><Assumption label="收入驱动" value={`IT设备 ${(assumptions.itGrowth * 100).toFixed(1)}% / 软件服务 ${(assumptions.servicesGrowth * 100).toFixed(1)}%`} /><Assumption label="毛利率" value={`IT设备 ${(assumptions.itGrossMargin * 100).toFixed(1)}% / 软件服务 ${(assumptions.servicesGrossMargin * 100).toFixed(1)}%`} /><Assumption label="现金流驱动" value={`CAPEX率 ${((assumptions.maintenanceCapexPct + assumptions.growthCapexPct) * 100).toFixed(1)}% / 应收 ${assumptions.arDays.toFixed(0)}天`} /></div>
        </section>
